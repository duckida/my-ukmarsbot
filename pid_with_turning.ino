...
ISR(TIMER2_COMPA_vect) {
  ...
  if (gSensorFront > 25) {
    setMotorPWM(0, 0);
    int oldValueLeft = encoderLeftCount;
    int oldValueRight = encoderRightCount;
    int encoderDifference = 0;
    /*while (encoderDifference < 100.5) {
      updateLeftEncoder();
      updateRightEncoder();
      setMotorVolts(1.0, -1.0);
      encoderDifference = (encoderRightCount - oldValueRight) - (encoderLeftCount - oldValueLeft);
