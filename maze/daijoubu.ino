...
// WALL SENSOR VALUES
uint8_t frontWallThreshold = 30;
uint8_t leftGapThreshold = 5;
uint8_t rightGapThreshold = 5;

float eDifference;

// PID CONSTANTS
float target = 8; // The target walldistance left
float rightTarget = 8; // The target walldistance right
...
void goACellForward() {
  ...
  leftAvg = leftTotal / 10;
  rightAvg = rightTotal / 10;
...
void mazeLoop() {
  if (state == 0) { // solving state
    ...
    uint16_t frontAvg = frontTotal / 10;
    ...
    switch (turningAngle) {
      ...
      case 90: 
        ...
        driveAngle(1.0, 1.0, -92); // turn right -95
        updateDirection(90); // update direction
        goACellForward();
        break;
      case 180:
